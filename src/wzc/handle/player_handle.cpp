#include "player_handle.h"
#include "wzc/game_state.h"
#include "wzc/game.h"

namespace wzc {
    PlayerHandle::PlayerHandle(const std::string& id,
                               Game* game)
            : id(id), game(game) { }
    
    const std::string& PlayerHandle::getId() const {
        return id;
    }
    
    const Game* PlayerHandle::getGame() const {
        return game;
    }
    
    bool PlayerHandle::isValid() const {
        return game != nullptr;
    }
    
    bool PlayerHandle::exists() const {
        return existsStaged();
    }
    
    GamePlayer& PlayerHandle::get() const {
        return getStaged();
    }
    
    bool PlayerHandle::existsStaged() const {
        if (!isValid()) {
            throw std::runtime_error("Handle is no longer valid");
        }
        
        GameState* stagedGameState = game->stagedGameState;
        
        if (stagedGameState == nullptr) {
            throw std::runtime_error("There is no staging in process");
        }
        
        return existsIn(stagedGameState);
    }
    
    bool PlayerHandle::existsUnstaged() const {
        if (!isValid()) {
            throw std::runtime_error("Handle is no longer valid");
        }
        
        GameState* unstagedGameState = game->gameState;
        
        if (unstagedGameState == nullptr) {
            // should never happen, but better safe than sorry
            throw std::runtime_error("Game does not have a game state");
        }
        
        return existsIn(unstagedGameState);
    }
    
    GamePlayer& PlayerHandle::getStaged() const {
        if (!isValid()) {
            throw std::runtime_error("Handle is no longer valid");
        }
        
        GameState* stagedGameState = game->stagedGameState;
        
        if (stagedGameState == nullptr) {
            throw std::runtime_error("There is no staging in process");
        }
        
        if (!existsStaged())
            throw std::runtime_error("Component does not exist");
        
        return getFrom(stagedGameState);
    }
    
    GamePlayer& PlayerHandle::getUnstaged() const {
        if (!isValid()) {
            throw std::runtime_error("Handle is no longer valid");
        }
        
        GameState* unstagedGameState = game->gameState;
        
        if (unstagedGameState == nullptr) {
            // should never happen, but better safe than sorry
            throw std::runtime_error("Game does not have a game state");
        }
        
        if (!existsUnstaged())
            throw std::runtime_error("Component does not exist");
        
        return getFrom(unstagedGameState);
    }
    
    bool PlayerHandle::existsIn(const GameState* gameState) const {
        return gameState->hasPlayer(id);
    }
    
    GamePlayer& PlayerHandle::getFrom(GameState* gameState) const {
        return gameState->getPlayer(id);
    }
}
